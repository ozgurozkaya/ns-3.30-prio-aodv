...
RoutingExperiment::Run (int nSinks, double txp, std::string CSVfileName)
{
  ...
  string experiment = "MRC-routing-test";
  string strategy = "MRC-test";
  string input;
  string runID = "MRC";
  ...
  std::string animFile = "manet-routing-compare-animation.xml";
  ...
  ApplicationContainer apps_sink[adhocNodes.GetN()];
  for(uint32_t tmpi = 0; tmpi < adhocNodes.GetN(); tmpi++){
    PacketSinkHelper sinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), port));
    apps_sink[tmpi] = sinkHelper.Install (adhocNodes.Get (tmpi));
    apps_sink[tmpi].Start (Seconds (0));
    apps_sink[tmpi].Stop (Seconds (TotalTime));
  }
  ...
  for (int i = 0; i < nSinks; i++)
    {
      ...
      totalTx[i]->SetKey ("MRC-appTx-packets");
      totalTx[i]->SetContext ("node-"+to_string(i + nSinks));
      Config::Connect ("/NodeList/" + to_string(i + nSinks) + "/ApplicationList/*/$ns3::OnOffApplication/Tx", 
      ...
      totalRx[i]->SetKey ("MRC-rx-packets");
      totalRx[i]->SetContext ("node-"+to_string(i));
      Config::Connect ("/NodeList/"+to_string(i)+"/ApplicationList/*/$ns3::PacketSink/Rx",
  ...
  internet.EnablePcapIpv4All ("xml/pcap/mrc");
